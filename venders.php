...
if(isset($save)){
    ...
    $qeury=mysqli_query($conn,"insert into venders values('','$fname','$lname','$phone','$gender','$adress')");
    echo "<script>window.location.href = 'venders.php';</script>";
}
    
if(isset($_GET['id'])){
    $id= $_GET['id'];
    $conn=mysqli_connect("localhost","root","","new"); 
    $qeury=mysqli_query($conn,"delete from venders where v_id='$id'");
    $results= $conn->query($qeury);
    if ($results) {
        echo "well sql";
        header("Location:venders.php");
    }
      else{
        echo "error ocurred in sql";
?> 
   <script>window.location.href = "venders.php";</script><?php
      }
